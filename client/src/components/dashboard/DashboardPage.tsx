...
export default function DashboardPage() {
    ...
    async function loadDashboard() {
        try {
            const [s, c] = await Promise.all([dashboardApi.stats(), dashboardApi.charts()]);
            setStats(s);
            setCharts(c);
        } catch (err) {
    ...
    const statCards = [
        { label: 'Task-uri active', value: stats?.active || 0, icon: Activity, color: 'from-blue-500 to-blue-600', textColor: 'text-blue-400' },
        { label: 'Depășite', value: stats?.overdue || 0, icon: AlertTriangle, color: 'from-red-500 to-red-600', textColor: 'text-red-400' },
        { label: 'Blocate', value: stats?.blocked || 0, icon: Ban, color: 'from-orange-500 to-orange-600', textColor: 'text-orange-400' },
        { label: 'Finalizate luna aceasta', value: stats?.completed_this_month || 0, icon: CheckCircle2, color: 'from-green-500 to-green-600', textColor: 'text-green-400' },
    ];
        ...
        <div className="p-6 space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {statCards.map((card, i) => (
                    <div key={i} className="bg-navy-900/50 border border-navy-700/50 rounded-xl p-5 hover:border-navy-600/50 transition-all animate-slide-up" style={{ animationDelay: `${i * 100}ms` }}>
            ...
            <div className="bg-navy-900/50 border border-navy-700/50 rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => navigate('/tasks')} className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
                        Vezi toate <ChevronRight className="w-3 h-3" />
                    </button>
                </div>

                {charts?.urgent_tasks && charts.urgent_tasks.length > 0 ? (
                    <div className="space-y-2">
                        {charts.urgent_tasks.map((task) => {
                            const dueStat = getDueDateStatus(task.due_date);
                            return (
                                <div
                                    key={task.id}
                                    onClick={() => navigate('/tasks', { state: { openTaskId: task.id } })}
                                    className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all hover:bg-navy-800/50 ${dueStat === 'overdue' ? 'border-l-2 border-red-500 bg-red-500/5' :
                                ...
                                >
                                    ...
                                    <div className="text-right flex-shrink-0">
                                        <p className={`text-xs font-medium ${dueStat === 'overdue' ? 'text-red-400' :
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="text-navy-500 text-sm">Niciun task urgent!</p>
                    </div>
                )}
            </div>
        </div>
