                            ...
                            displayed.map((group, idx) => {
                                ...
                                const sectionHeader =
                                    idx === 0 && group.hasUnread ? (
                                        <div key="hdr-unread" className={`px-4 pt-3 pb-2 text-[11px] font-bold uppercase tracking-[0.14em] ${darkMode ? 'text-blue-300' : 'text-blue-500'}`}>
                                            {t('notif.section_unread')}
                                        </div>
                                    ) : (!group.hasUnread && (idx === 0 || displayed[idx - 1].hasUnread)) ? (
                                        <div key="hdr-read" className={`mt-3 px-4 pt-3 pb-2 text-[11px] font-bold uppercase tracking-[0.14em] border-t ${darkMode ? 'text-navy-500 border-navy-600/50 bg-navy-900/40' : 'text-gray-400 border-gray-200 bg-gray-50'}`}>
                                ...
                                return (
                                    ...
                                    >
                                        {/* Group header */}
                                        <div
                                            className="px-4 py-2.5 cursor-pointer flex items-center gap-2.5 hover:bg-navy-700/30 transition-colors"
                                        ...
                                        >
                                            ...
                                            {group.hasUnread && <span className="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0" style={{ boxShadow: '0 0 0 3px rgba(59,130,246,.18)' }} />}
                                            <span className={`text-[10px] flex-shrink-0 ${darkMode ? 'text-navy-500' : 'text-gray-300'}`}>{timeAgoShort(group.latestAt)}</span>
                                            <ChevronRight className={`w-4 h-4 flex-shrink-0 text-navy-500 transition-transform ${isExpanded ? 'rotate-90' : ''}`} />
                                        </div>
