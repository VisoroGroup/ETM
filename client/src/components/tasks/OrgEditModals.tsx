        ...
        <div className="flex items-center gap-2 pt-2">
            {onDelete && (
                <button onClick={onDelete} className="flex items-center gap-1 px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
                    <Trash2 className="w-3.5 h-3.5" /> {t('org.deactivate')}
                </button>
            )}
            <div className="flex-1" />
            <button onClick={onClose} className="px-4 py-2 text-sm text-navy-400 hover:text-white transition-colors">{t('common.cancel')}</button>
...
export function DepartmentEditModal({ department, onClose, onSaved }: { department: OrgDepartment; onClose: () => void; onSaved: () => void }) {
    ...
    const handleDelete = async () => {
        if (!confirm(t('org.confirm_deactivate_dept'))) return;
        try { await departmentsApi.delete(department.id); onSaved(); onClose(); } catch (err) { console.error(err); }
    };

    return (
        <ModalWrapper title={t('org.edit_prefix', { name: department.name })} onClose={onClose}>
            <InputField label={t('common.name')} value={name} onChange={setName} />
            <div>
                <label className="text-xs font-medium text-navy-400 mb-1 block">{t('common.color')}</label>
            ...
            <div>
                <label className="text-xs font-medium text-navy-400 mb-1 block">{t('org.dept_head')}</label>
                <select value={headUserId} onChange={e => setHeadUserId(e.target.value)} className="w-full px-3 py-2 bg-navy-800/50 border border-navy-700/50 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500/50">
                    <option value="">{t('org.nobody')}</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.display_name}</option>)}
                </select>
            </div>
            <InputField label={t('org.pfv')} value={pfv} onChange={setPfv} multiline />
            <InputField label={t('org.statistic')} value={statisticName} onChange={setStatisticName} />
...
export function SectionEditModal({ section, onClose, onSaved }: { section: OrgSection; onClose: () => void; onSaved: () => void }) {
    ...
    const handleDelete = async () => {
        if (!confirm(t('org.confirm_deactivate_section'))) return;
        try { await sectionsApi.delete(section.id); onSaved(); onClose(); } catch (err) { console.error(err); }
    };

    return (
        <ModalWrapper title={t('org.edit_section_prefix', { name: section.name })} onClose={onClose}>
            <InputField label={t('common.name')} value={name} onChange={setName} />
            <div>
                <label className="text-xs font-medium text-navy-400 mb-1 block">{t('org.section_head')}</label>
                <select value={headUserId} onChange={e => setHeadUserId(e.target.value)} className="w-full px-3 py-2 bg-navy-800/50 border border-navy-700/50 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500/50">
                    <option value="">{t('org.nobody')}</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.display_name}</option>)}
                </select>
...
export function PostEditModal({ post, onClose, onSaved }: { post: OrgPost; onClose: () => void; onSaved: () => void }) {
    ...
    const handleDelete = async () => {
        if (!confirm(t('org.confirm_deactivate_post'))) return;
        try { await postsApi.delete(post.id); onSaved(); onClose(); } catch (err) { console.error(err); }
    };

    return (
        <ModalWrapper title={t('org.edit_post_prefix', { name: post.name })} onClose={onClose}>
            <InputField label={t('task_form.post_name')} value={name} onChange={setName} />
            <div>
                <label className="text-xs font-medium text-navy-400 mb-1 block">{t('org.post_head')}</label>
                <select value={userId} onChange={e => setUserId(e.target.value)} className="w-full px-3 py-2 bg-navy-800/50 border border-navy-700/50 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500/50">
                    <option value="">{t('org.unoccupied_dash')}</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.display_name}</option>)}
                </select>
                {userId !== (post.user_id || '') && userId && post.user_id && (
                    <p className="text-[10px] text-amber-400 mt-1">
                        {t('org.post_head_change_warning')}
                    </p>
                )}
            </div>
            <InputField label={t('tasks.description')} value={description} onChange={setDescription} multiline placeholder={t('org.post_description_placeholder')} />
            <ActionButtons saving={saving} onSave={handleSave} onDelete={handleDelete} onClose={onClose} />
        </ModalWrapper>
    );
}
