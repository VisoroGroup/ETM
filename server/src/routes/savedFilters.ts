...
router.get('/', async (req: AuthRequest, res: Response) => {
    try {
        ...
        console.error('Error listing saved filters:', err);
        res.status(500).json({ error: 'Eroare la încărcarea filtrelor salvate.' });
    }
});
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.status(500).json({ error: 'Eroare la ștergerea filtrului.' });
    }
});
